...
namespace HealthCareSysAPI.Controllers
{
    ...
    public class DoctorController : ControllerBase
    {
        ...
        [HttpGet("ShowAcceptedDoctorAppointment")]
        public IActionResult ShowAcceptedDoctorAppointment(string doctorID)
        {
            var appointments = _dbContext.Appointments.Where(x => x.DoctorID == doctorID && x.IsAccepted == true).ToList();
            if (appointments != null)
            {
                foreach (var appointment in appointments)
                {
